...
Both are kids and are making some mistakes in typing and are making use 
of the '#' key on a typewriter to delete the last character printed on it. 
...
Output Format
The first line contains 'YES' if Alice is able to print the exact 
words as Bob, otherwise 'NO'.
...
The + operator is the simplest and most straightforward way 
...
Cons:
Inefficient for large numbers of concatenations: Each time you 
concatenate two strings using +, a new string is created. 
This can become inefficient if you're concatenating strings 
in a loop or doing multiple concatenations.'''