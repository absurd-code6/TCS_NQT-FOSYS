...
Input: arr[] = [3, 0, 2, 0, 4]
Output: 7
Explanation: We trap 3+1+3= 7 units.
...
# We define the range from idx 1 element to the 2nd last element 
#bcoz the 0-idx element has no left side to calculate left_max
#Similarly the last element(idx:len(arr)-1) has no right side 
# to calculate the right_max
        left=arr[i]
        for j in range(i):
            left=max(left,arr[j])
