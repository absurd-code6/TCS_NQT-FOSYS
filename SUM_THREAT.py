...
action is performed 'R' number of times. 
The resultant is also a single digit that is yet to be deciphered. 
The task here is to find the single-digit sum of the given number 'N' 
by repeating the action 'R' number of times.
...
The output should be a positive integer number or print 
the message 
(if any) given in the problem statement. 
(Check the output in Example 1, Example 2)'''

n=input("Enter the number:")
#n is taken as a string (important, so you can iterate over digits)
...
while len(n)>1:
    ...
    for i in n:
        sum+=int(i)
    n=str(sum)
#This repeatedly sums digits until only one digit remains.
