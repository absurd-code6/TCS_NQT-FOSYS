...
# Start with the value of the first Roman numeral
    Int=roman_letters[num[0]] # Impt
    for i in range(1,len(num)):
        if num[i] in roman_letters:
            Int+=roman_letters[num[i]]
#If a smaller numeral precedes a larger one (e.g., IV or IX),
#subtract twice the previous value (since it was added earlier)
        if roman_letters[num[i]]>roman_letters[num[i-1]]:
            Int-=2*roman_letters[num[i-1]]
    return Int
